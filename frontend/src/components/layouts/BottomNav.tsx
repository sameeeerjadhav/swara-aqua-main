...
// ── Admin bottom nav — 5 slots: 2 left | center | 1 right | More ─────────────
const AdminBottomNav = ({ items }: { items: NavItem[] }) => {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const { pathname } = useLocation();

  // Slots: [0]=left1, [1]=left2, [2]=center, [3]=right1, [4+]=overflow
  const primary  = items.slice(0, 4);   // 4 tabs
  const overflow = items.slice(4);       // everything else in More
    ...
    <>
      ...
      >
        <div className="flex justify-around items-end px-1 pt-2 pb-2">
          {/* 4 primary tabs — all identical style, center is just index 2 */}
