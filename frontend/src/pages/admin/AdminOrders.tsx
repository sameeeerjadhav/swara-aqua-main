...
// ── Payment mode labels ─────────────────────────────────────────────────────
const PM_LABEL: Record<string, { label: string; icon: string; cls: string }> = {
...
const PaymentEditModal = ({ order, onSaved, onClose }: PaymentEditModalProps) => {
  const { toast } = useToast();
  const [mode, setMode]     = useState<'cash'|'online'|'advance'|'pay_later'>(
    (order.delivery_payment_mode as any) || 'cash'
  );
  const [amount, setAmount] = useState(Number(order.delivery_collected_amount ?? order.total_amount ?? 0));
  const [saving, setSaving] = useState(false);

  const handleSave = async () => {
    if (!order.delivery_id) { toast('No delivery record found', 'error'); return; }
    setSaving(true);
    try {
      const payload: DeliveryPaymentPayload = { payment_mode: mode, collected_amount: mode === 'pay_later' ? 0 : amount };
      await calendarApi.updateDeliveryPayment(order.delivery_id, payload);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <p className="text-sm font-bold text-slate-800">✏️ Correct Payment — Order #{order.id}</p>
            <p className="text-xs text-slate-400 mt-0.5">{order.customer_name} · {order.quantity} jars · ₹{order.total_amount}</p>
          </div>
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          {order.delivery_payment_mode && (
            <div className="flex items-center gap-2 text-xs text-slate-500">
              ...
              <span className={`inline-flex items-center gap-1 font-bold px-2 py-0.5 rounded-full border ${
                PM_LABEL[order.delivery_payment_mode]?.cls ?? ''
              }`}>
                {PM_LABEL[order.delivery_payment_mode]?.icon} {PM_LABEL[order.delivery_payment_mode]?.label}
              </span>
              <span>· ₹{order.delivery_collected_amount ?? 0} collected</span>
            </div>
          )}
              ...
              {PAYMENT_MODES.map(pm => (
                <button key={pm.value} onClick={() => setMode(pm.value)}
                  className={`flex items-center gap-2 py-2.5 px-3 rounded-xl border text-sm font-semibold transition-all ${
                    mode === pm.value ? 'bg-brand-600 text-white border-brand-600 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-brand-300'
                  }`}>
          ...
          {mode === 'pay_later' && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800">
              ⚠️ Amount collected will be set to ₹0. Customer's pending balance will increase by ₹{order.total_amount}.
            </div>
          )}
                  ...
                  <td className="px-4 py-3.5 text-xs text-slate-400 whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      ...
                      {o.delivery_id && o.delivery_status === 'delivered' && (
                        <button
                          onClick={e => { e.stopPropagation(); setEditingPaymentOrder(o); }}
          ...
          {loading ? (
            ...
            <motion.div key={o.id}
              ...
              <div className="flex gap-2 mt-3">
                ...
                {o.delivery_id && o.delivery_status === 'delivered' && (
                  <button
                    onClick={() => setEditingPaymentOrder(o)}
