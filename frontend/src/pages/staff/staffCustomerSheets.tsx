/**
 * Shared staff customer sheet components — imported by StaffCustomers and StaffGroups.
 * Contains: CalendarModal, SuccessCard, DeliverySheet, ProfileSheet, CallPhone, openMaps
 */
import { useState, useCallback, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  Phone, MapPin, Package, IndianRupee, CheckCircle2, X,
  Droplets, Clock, Navigation, ChevronRight, Eye,
...
export type PaymentMode = 'cash' | 'online' | 'pay_later';

export const MODE_OPTIONS: { id: PaymentMode; label: string; color: string; bg: string }[] = [
...
// Navigate helper
export const openMaps = (c: CustomerForStaff, e: React.MouseEvent) => {
  e.stopPropagation();
  const query = c.address ? encodeURIComponent(c.address) : encodeURIComponent(c.name);
  window.open(`https://www.google.com/maps/dir/?api=1&destination=${query}`, '_blank');
};

// Call phone
export const CallPhone = ({ phone }: { phone: string }) => (
  <a
    href={`tel:${phone}`}
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)}
              placeholder={t('notes_ph', lang)} rows={2}
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={onCalendar}
              className="w-8 h-8 flex items-center justify-center rounded-full bg-brand-50 text-brand-600 hover:bg-brand-100 transition-colors" title="View delivery calendar">
        ...
        <div className="flex flex-col items-center px-5 pb-5 pt-2 border-b border-slate-100">
          ...
          <div className="mt-2"><CallPhone phone={customer.phone} /></div>
        </div>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div className={`flex items-center justify-between rounded-2xl px-4 py-3 ${Number(customer.today_jars) > 0 ? 'bg-green-50' : 'bg-slate-50'}`}>
            <div className="flex items-center gap-2">
...
// Calendar modal
export const CalendarModal = ({ customer, onClose }: { customer: CustomerForStaff; onClose: () => void }) => {
          ...
          {!selectedDay ? (
                    ...
                    {weeks.map((wk, wi) => (
                      <div key={wi} className="grid grid-cols-7 gap-1.5">
                        {wk.map((cell, di) => {
                          ...
                          return (
                            <button key={di} onClick={() => handleDayClick(cell)} disabled={!hasDelivery}
                              ...
                              <p className={`text-[11px] font-bold leading-none ${hasDelivery ? 'text-green-700' : 'text-slate-400'}`}>{cell.day}</p>
            ...
            <motion.div key="daydetail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
              ...
              <div className="overflow-y-auto flex-1 px-4 pb-5 pt-4 space-y-3">
                {dayLoading ? (
                  <div className="space-y-3">{[0,1,2].map(i => <div key={i} className="h-20 rounded-2xl bg-slate-100 animate-pulse" />)}</div>
